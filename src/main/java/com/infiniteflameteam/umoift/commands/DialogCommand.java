...
public class DialogCommand {
    ...
    private static int executeShowDialog(CommandContext<CommandSourceStack> context,
        ...
        try {
            // 检查是否选中了玩家
            if (targets.isEmpty()) {
                context.getSource().sendFailure(LanguageManager.literalComponent("umoift.error.no_targets"));
                return 0;
            }
            ...
            if (successCount > 0) {
                context.getSource().sendSuccess(() ->
                                LanguageManager.literalComponent("umoift.command.dialog.show_success", finalSuccessCount),
                        true);
                return successCount;
            } else {
                context.getSource().sendFailure(
                        LanguageManager.literalComponent("umoift.command.dialog.show_fail", dialogInput)
                );
                return 0;
            }
        } catch (Exception e) {
            LOGGER.error("显示对话框时出错", e);
            context.getSource().sendFailure(
                    LanguageManager.literalComponent("umoift.error.generic", e.getMessage())
            );
            return 0;
        }
    }
    ...
    private static JsonObject parseSnbtToJson(String snbt) {
        try {
            ...
            if (!jsonString.startsWith("{") || !jsonString.endsWith("}")) {
                throw new IllegalArgumentException(LanguageManager.translate("umoift.error.snbt_parse", "无效的SNBT格式"));
            }
            ...
            throw new IllegalArgumentException(LanguageManager.translate("umoift.error.snbt_parse", e.getMessage()));
        }
    }
    ...
    private static int executeClearDialog(CommandContext<CommandSourceStack> context,
        ...
        try {
            ...
            context.getSource().sendSuccess(() ->
                            LanguageManager.literalComponent("umoift.command.dialog.clear_success", finalClearedCount),
                    true);
            return clearedCount;
        } catch (Exception e) {
            context.getSource().sendFailure(LanguageManager.literalComponent("umoift.command.dialog.clear_fail", e.getMessage()));
            return 0;
        }
    }
    ...
    private static int executeListOfficial(CommandContext<CommandSourceStack> context) {
        ...
        context.getSource().sendSuccess(() ->
                        LanguageManager.literalComponent("umoift.command.dialog.list", dialogsList),
                false);
        return dialogCount;
    }
    ...
    private static int executeReloadOfficial(CommandContext<CommandSourceStack> context) {
        try {
            ...
            context.getSource().sendSuccess(() ->
                            LanguageManager.literalComponent("umoift.command.dialog.reload_success"),
                    true);
            return 1;
        } catch (Exception e) {
            context.getSource().sendFailure(
                    LanguageManager.literalComponent("umoift.command.dialog.reload_fail", e.getMessage())
            );
            return 0;
        }
    }
